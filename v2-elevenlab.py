...
def stream_tts_to_mp3(voice_id: str, text: str, out_path: Path) -> None:
    ...
    Also writes the transcription to a TXT file and an SRT file with timestamps.
    """
                        ...
                        if char == ' ':
                            if word:
                                words.append({
                                    ...
                                    'end_time': character_start_times_seconds[i]
                                })
                                word = ''
                        else:
                            if not word:
                                word_start_time = character_start_times_seconds[i]
                            word += char
                    if word:
                        words.append({
                            'text': word,
                            'start_time': word_start_time,
                            'end_time': character_end_times_seconds[-1]
                        })

                    for word in words:
                        start_time = format_time(word['start_time'])
                        end_time = format_time(word['end_time'])
                        srt_content += f"{chunk_index}\n{start_time} --> {end_time}\n{word['text']}\n\n"
                        txt_content += word['text'] + " "
                        chunk_index += 1
    ...
    try:
        ...
        print("✔ Wrote transcription to output.srt and output.txt\n")
    except Exception as fe:
        print(f"[FILE WRITE ERROR] {fe}")
        sys.exit(1)
